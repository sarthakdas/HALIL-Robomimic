...
class OpenAIClient:
        ...
        for attempt in range(max_attempts):
            try:
                response = self.client.chat.completions.create(
                    model='gpt-4o-mini',  # Consider updating the model if using an updated API: gpt-3.5-turbo, gpt-4o, gpt-4o-mini
    ...
    def process_test(self, demonstation_file_path, user_prompt, scene_prompt, context_description):
    ...
    def process_ensemble_training(self, demonstation_file_path, user_prompt, scene_prompt, context_description, n=1 ,temperature=0):
        ...
        generated_pathways = []
        for i in range(n):
            generated_pathways.append(self.extract_waypoints(response.choices[i].message.content))
        ...
        self.system_prompt = self.load_prompt(demonstation_file_path)
        context_description = "baed on the demonsations above, pick the correct option below and repsond with the letter A,B,C,D,E, Only respond with a singular letter"
        user_prompt = "Give the best answer based on the options below"
        self.update_system_prompt(user_prompt, scene_prompt, context_description)
        ...
        for token, logprob in zip(top_tokens, mc_smx_all):
            print('Option:', token, '\t', 'log prob:', logprob)
            log_probs_dict[token] = logprob

        # attach the log probabilities to the generated pathways
        ...
        # convert char to int
        prediction_set = [ord(token) - 65 for token in prediction_set]

        pathways = []
        for i in range(n):
            # convert i to char beinging from A 
            l = chr(65+i)
            if i in prediction_set:
                pathways.append({"action_token_trajectory":generated_pathways[i], "logprob": log_probs_dict[l], "prediction_set": True})
            else:
                pathways.append({"action_token_trajectory":generated_pathways[i], "logprob": log_probs_dict[l], "prediction_set": False})
